...
/*!
  @file       gomin.h
  @brief      GOMin template definition
...
class GOMin : public ELT
{
  protected :
    ...
    void           hookup( ELT &elt )
                   {
                     elt.valueCB().install( new Observer<GOMin>( this, GOMin::onDataChange )) ;
                     if ((*this > (ELTTYPE)elt) || (_vec->size() == 1))
                     {
    ...
    void           hookup() 
                   { 
                     ...
                     _vec->postInsertCB().install( new Observer<GOMin>( this, GOMin::onVectorAdd ) ) ;
                     _vec->preEraseCB  ().install( new Observer<GOMin>( this, GOMin::onVectorRemove ) ) ;
